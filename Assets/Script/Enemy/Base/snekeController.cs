...
public class snekeController : MonoBehaviour
{
	...
	void Update()
	{
		Transform _player = player;

		float distance = Vector3.Distance(enemypos.position, _player.transform.position);//敵とプレイヤーの距離を求める
		...
		if (_player != null)
		{
			if (distance > rangeOfCaution)
			{
				...
				anim.SetBool("snekeAttack", false);
				move();
